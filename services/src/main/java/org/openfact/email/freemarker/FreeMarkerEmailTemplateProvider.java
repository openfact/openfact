...
public class FreeMarkerEmailTemplateProvider implements EmailTemplateProvider {
	...
	public void sendEvent(Event event) throws EmailException {
		...
		send(toCamelCase(event.getType()) + "Subject", "event-" + event.getType().toString().toLowerCase() + ".ftl",
				attributes, null);
	}
	...
	private void send(String subjectKey, String template, Map<String, Object> attributes, List<AttachModel> attachments)
	...
	public void sendInvoice(InvoiceModel invoice) throws EmailException {
		...
		attachments.add(new AttachModel() {
			...
			public String getMimiType() {
				return InternetMediaType.XML.getMimiType();
			}
		...
		send(toCamelCase(EventType.INVOICE) + "Subject", "event-" + EventType.INVOICE.toString().toLowerCase() + ".ftl",
