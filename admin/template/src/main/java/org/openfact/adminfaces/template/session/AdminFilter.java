package org.openfact.adminfaces.template.session;

import org.openfact.adminfaces.template.config.AdminConfig;
import org.openfact.adminfaces.template.util.Constants;
import org.openfact.adminfaces.template.util.Assert;
...
import static org.openfact.adminfaces.template.util.Assert.has;
...
public class AdminFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        if (adminConfig.isDisableFilter() || Assert.has(disableAdminFilter) && Boolean.valueOf(disableAdminFilter)) {
            disableFilter = true;
        }
        if (!disableFilter) {
            try {
                loginPage = filterConfig.getServletContext().getInitParameter(Constants.InitialParams.LOGIN_PAGE);
                if (!Assert.has(loginPage)) {
                    loginPage = Assert.has(adminConfig) ? adminConfig.getLoginPage() : Constants.DEFAULT_LOGIN_PAGE;
                }
                errorPage = filterConfig.getServletContext().getInitParameter(Constants.InitialParams.ERROR_PAGE);
                if (!Assert.has(errorPage)) {
                    errorPage = Assert.has(adminConfig) ? adminConfig.getErrorPage() : Constants.DEFAULT_ERROR_PAGE;
                }
                indexPage = filterConfig.getServletContext().getInitParameter(Constants.InitialParams.INDEX_PAGE);
                if (!Assert.has(indexPage)) {
                    indexPage = Assert.has(adminConfig) ? adminConfig.getIndexPage() : Constants.DEFAULT_INDEX_PAGE;
                }
    ...
    public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws IOException, ServletException {
        ...
        if (skipResource(request) || adminSession.isLoggedIn()) {
            if (!adminSession.isUserRedirected() && adminSession.isLoggedIn() && Assert.has(request.getHeader("Referer")) && request.getHeader("Referer").contains("?page=")) {
    ...
    private void redirectToLogon(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if (Assert.has(referer) && referer.contains("?")) {
            ...
            */
            String redirectUrl = request.getContextPath() + "/" + loginPage + (Assert.has(recoveryUrl) &&
