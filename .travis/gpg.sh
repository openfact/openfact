...
cat >gen-key-script <<EOF
      ...
      Subkey-Length: 4096
      Name-Real: Openfact
      Name-Email: carlosthe19916@sistcoop.com
      Expire-Date: 2y
...
$GPG_EXECUTABLE --batch --gen-key gen-key-script
...
export GPG_KEYNAME=$($GPG_EXECUTABLE -K | grep ^sec | cut -d/  -f2 | cut -d\  -f1 | head -n1)

# cleanup local configuration
shred gen-key-script

# publish the gpg key  (sonatype reads from keyserver.ubuntu.com)
$GPG_EXECUTABLE --keyserver keyserver.ubuntu.com --send-keys ${GPG_KEYNAME}

# wait for the key beeing accessible
while(true); do
    date
    $GPG_EXECUTABLE --keyserver keyserver.ubuntu.com --recv-keys ${GPG_KEYNAME} && break || sleep 30
done