...
public class ModelToRepresentation {
	...
	public static OrganizationRepresentation toRepresentation(OrganizationModel organization, boolean internal) {
		...
		rep.setAdditionalAccountId(organization.getAdditionalAccountId() != null
				? organization.getAdditionalAccountId().toString() : null);
		...
		rep.setPostalAddress(
				organization.getPostalAddress() != null ? toRepresentation(organization.getPostalAddress()) : null);

		if (internal) {
			rep.setTasksSchedule(
					organization.getTasksSchedule() != null ? toRepresentation(organization.getTasksSchedule()) : null);
		}
		return rep;
	}
	...
	public static InvoiceRepresentation toRepresentation(InvoiceModel invoice) {
		...
		rep.setTotalTaxed(invoice.getAdditionalInformation() != null
				? invoice.getAdditionalInformation().get(AdditionalInformationType.GRAVADO) : null);
		rep.setTotalUnaffected(invoice.getAdditionalInformation() != null
				? invoice.getAdditionalInformation().get(AdditionalInformationType.INACFECTO) : null);
		rep.setTotalExonerated(invoice.getAdditionalInformation() != null
				? invoice.getAdditionalInformation().get(AdditionalInformationType.EXONERADO) : null);
		rep.setTotalByFree(invoice.getAdditionalInformation() != null
				? invoice.getAdditionalInformation().get(AdditionalInformationType.GRATUITO) : null);
		rep.setTotalDiscounted(invoice.getTotalLegalMonetary() != null
				? invoice.getTotalLegalMonetary().get(MonetaryTotalType.DESCUENTO_TOTAL) : null);
		rep.setTotalAmmount(invoice.getTotalLegalMonetary() != null
				? invoice.getTotalLegalMonetary().get(MonetaryTotalType.IMPORTE_TOTAL) : null);
		...
		invoice.getInvoiceLines()
				.forEach(invoiceLineModel -> representationList.add(toRepresentation(invoiceLineModel)));
	...
	private static CustomerRepresentation toRepresentation(CustomerModel customer) {
		...
		rep.setAdditionalAccountId(
				customer.getAdditionalAccountId() != null ? customer.getAdditionalAccountId().getDescription() : null);
