...
public class JpaContadorSerieNumeroPEProvider implements ContadorSerieNumeroPEProvider {
    ...
    private Optional<ContadorSerieNumeroPEEntity> getPresente(OrganizationModel organizacion, String tipoDocumento, String serie) {
        TypedQuery<ContadorSerieNumeroPEEntity> q = em.createNamedQuery("getContadorByOrganizacionIdTipoDocumentoAndSerie", ContadorSerieNumeroPEEntity.class);
    ...
    public ContadorSerieNumeroPEModel getSiguiente(OrganizationModel organization, String tipoDocumento, String serie) {
        ...
        Optional<ContadorSerieNumeroPEEntity> optional = getPresente(organization, tipoDocumento, serie);
        if (optional.isPresent()) {
            ContadorSerieNumeroPEEntity entity = optional.get();
            int numeroActual = entity.getNumero();
            if (NUMERO_MAXIMO.orElse(99_999_999) > numeroActual) {
                entity.setNumero(numeroActual + 1);
                em.merge(entity);
                em.flush();
                return new ContadorSerieNumeroPEAdapter(em, entity);
            } else {
            ...
            ContadorSerieNumeroPEEntity entity = new ContadorSerieNumeroPEEntity();
