...
import org.openfact.models.ubl.cache.CreditNoteCacheProviderFactory;

/**
 * @author <a href="mailto:sthorger@redhat.com">Stian Thorgersen</a>
 */
public class InfinispanCreditNoteCacheProviderFactory implements CreditNoteCacheProviderFactory {

    private static final Logger log = Logger.getLogger(InfinispanCreditNoteCacheProviderFactory.class);

    protected volatile CreditNoteCacheManager userCache;

    @Override
    public CreditNoteCache create(OpenfactSession session) {
        lazyInit(session);
        return new CreditNoteCacheSession(userCache, session);
    }
        ...
        if (userCache == null) {
            synchronized (this) {
                if (userCache == null) {
                    ...
                    userCache = new CreditNoteCacheManager(cache, revisions);
                }
            }
        }
    }
