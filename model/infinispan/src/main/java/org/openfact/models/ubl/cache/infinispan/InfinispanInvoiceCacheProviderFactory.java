 ...
 */
public class InfinispanInvoiceCacheProviderFactory implements InvoiceCacheProviderFactory {

    private static final Logger log = Logger.getLogger(InfinispanInvoiceCacheProviderFactory.class);

    protected volatile InvoiceCacheManager invoiceCache;

    @Override
    public InvoiceCache create(OpenfactSession session) {
        lazyInit(session);
        return new InvoiceCacheSession(invoiceCache, session);
    }
        ...
        if (invoiceCache == null) {
            synchronized (this) {
                if (invoiceCache == null) {
                    ...
                    invoiceCache = new InvoiceCacheManager(cache, revisions);
                }
            }
        }
    }
