...
@Entity
@Table(name = "DEBIT_NOTE_SEND_EVENT")
@NamedQueries({
		@NamedQuery(name = "getAllSendEventsByDebitNote", query = "select i from DebitNoteSendEventEntity i join i.debitNotes a where a.id = :debitNoteId order by i.createdTimestamp") })
public class DebitNoteSendEventEntity {
	@Id
	@Column(name = "ID")
