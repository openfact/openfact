...
public class HazardousItemAdapter implements HazardousItemModel, JpaModel<HazardousItemEntity> {
	...
	public HazardousItemAdapter(OpenfactSession session, EntityManager em, HazardousItemEntity hazardousItem) {
	...
	public List<SecondaryHazardModel> getSecondaryHazard() {
		return hazardousItem.getSecondaryHazard().stream().map(f -> new SecondaryHazardAdapter(session, em, f))
				.collect(Collectors.toList());
	}

	@Override
	public void setSecondaryHazard(List<SecondaryHazardModel> secondaryHazard) {
		List<SecondaryHazardEntity> entities = secondaryHazard.stream().map(f -> SecondaryHazardAdapter.toEntity(f, em))
				.collect(Collectors.toList());
	...
	public List<TemperatureModel> getAdditionalTemperature() {
		return hazardousItem.getAdditionalTemperature().stream().map(f -> new TemperatureAdapter(session, em, f))
				.collect(Collectors.toList());
	}

	@Override
	public void setAdditionalTemperature(List<TemperatureModel> additionalTemperature) {
		List<TemperatureEntity> entities = additionalTemperature.stream().map(f -> TemperatureAdapter.toEntity(f, em))
				.collect(Collectors.toList());
