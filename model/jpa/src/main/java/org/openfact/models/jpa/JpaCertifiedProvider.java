...
public class JpaCertifiedProvider extends AbstractHibernateStorage implements CertifiedProvider {
    ...
    @Override
    public CertifiedModel addCertified(OrganizationModel organization, String alias, FileInputStream certificate, String password, LocalDate validity) {
        return addCertified(organization, OpenfactModelUtils.generateId(), alias, certificate, password, validity);
    }

    private CertifiedModel addCertified(OrganizationModel organization, String id, String alias, FileInputStream certificate, String password, LocalDate validity) {
        if (id == null) {
        ...
        entity.setCertificate("Ubicacion del certificado");
    ...
    @Override
    public CertifiedModel getCertifiedByAliasAndValidity(String alias, LocalDate validity, OrganizationModel organization) {
        TypedQuery<CertifiedEntity> query = em.createNamedQuery("getOrganizationCertifiedByAliasAndValidity", CertifiedEntity.class);
    ...
    public boolean disabledCertified(OrganizationModel organization, CertifiedModel certified) {
        ...
        updateCertified(certifiedEntity);
        return true;
    }

    private void updateCertified(CertifiedEntity certifiedEntity) {
        String id = certifiedEntity.getId();
        certifiedEntity = em.find(CertifiedEntity.class, id);
        if (certifiedEntity != null) {
            certifiedEntity.setEnabled(false);
            em.refresh(certifiedEntity);
        }
        em.flush();
    }
    ...
    public List<CertifiedModel> getCertifieds(OrganizationModel organization, Integer firstResult, Integer maxResults) {
        TypedQuery<CertifiedEntity> query = em.createNamedQuery("getAllCertifiedsByOrganization", CertifiedEntity.class);
        query.setParameter("organizationId", organization.getId());
        if (firstResult != -1) {
    ...
    @Override
    public List<CertifiedModel> searchForCertified(String filterText, OrganizationModel organization, Integer firstResult, Integer maxResults) {
    ...
    @Override
    public List<CertifiedModel> searchForCertifiedByAttributes(Map<String, String> attributes, OrganizationModel organization) {
        return searchForCertifiedByAttributes(attributes, organization, -1, -1);
    }

    @Override
    public List<CertifiedModel> searchForCertifiedByAttributes(Map<String, String> attributes, OrganizationModel organization, Integer firstResult, Integer maxResults) {
        StringBuilder builder = new StringBuilder("select i from CertifiedEntity i where u.organizationId = :organizationId");
    ...
    @Override
    public SearchResultsModel<CertifiedModel> search(OrganizationModel organization, SearchCriteriaModel criteria, String filterText) {
