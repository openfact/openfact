...
public class InvoiceLineAdapter implements InvoiceLineModel, JpaModel<InvoiceLineEntity> {
	...
	public List<OrderLineReferenceModel> getOrderLineReference() {
		return invoiceLine.getOrderLineReference().stream().map(f -> new OrderLineReferenceAdapter(session, em, f))
	...
	public List<LineReferenceModel> getDespatchLineReference() {
		return invoiceLine.getDespatchLineReference().stream().map(f -> new LineReferenceAdapter(session, em, f))
				.collect(Collectors.toList());
	}
	...
	public List<LineReferenceModel> getReceiptLineReference() {
		return invoiceLine.getReceiptLineReference().stream().map(f -> new LineReferenceAdapter(session, em, f))
				.collect(Collectors.toList());
	...
	public List<BillingReferenceModel> getBillingReference() {
		return invoiceLine.getBillingReference().stream().map(f -> new BillingReferenceAdapter(session, em, f))
				.collect(Collectors.toList());
	}
	...
	public List<DocumentReferenceModel> getDocumentReference() {
		return invoiceLine.getDocumentReference().stream().map(f -> new DocumentReferenceAdapter(session, em, f))
				.collect(Collectors.toList());
	}
