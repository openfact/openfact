...
public class ItemLocationQuantityAdapter implements ItemLocationQuantityModel, JpaModel<ItemLocationQuantityEntity> {
	...
	public void setApplicableTerritoryAddress(List<AddressModel> applicableTerritoryAddress) {
		List<AddressEntity> entities = applicableTerritoryAddress.stream().map(f -> AddressAdapter.toEntity(f, em))
	...
	public List<DeliveryUnitModel> getDeliveryUnit() {
		return itemLocationQuantity.getDeliveryUnit().stream().map(f -> new DeliveryUnitAdapter(session, em, f))
				.collect(Collectors.toList());
	...
	public List<TaxCategoryModel> getApplicableTaxCategory() {
		return itemLocationQuantity.getApplicableTaxCategory().stream().map(f -> new TaxCategoryAdapter(session, em, f))
				.collect(Collectors.toList());
	}

	@Override
	public void setApplicableTaxCategory(List<TaxCategoryModel> applicableTaxCategory) {
		List<TaxCategoryEntity> entities = applicableTaxCategory.stream().map(f -> TaxCategoryAdapter.toEntity(f, em))
				.collect(Collectors.toList());
