...
public class WsdlValidator {
		...
		try {
			...
			if (errors.isEmpty()) {
				if (bindingOperation == null) {
					errors.add(XmlError
							.forMessage("Missing operation [" + bindingOperation.getName() + "] in wsdl definition"));
				} else {
					...
					// validateInputAttachments(request, errors,
					// bindingOperation,
					// inputParts);
				}
			}
				...
				if (errors.isEmpty()) {
					if (bindingOperation == null) {
						errors.add(XmlError.forMessage(
								"Missing operation [" + bindingOperation.getName() + "] in wsdl definition"));
					} else {
	...
	@SuppressWarnings("unchecked")
	private void validateMessage(String message, BindingOperation bindingOperation, Part[] parts, List<XmlError> errors,
			boolean isResponse, boolean strict) {
		try {
			if (!wsdlContext.hasSchemaTypes()) {
				...
				if (!WsdlUtils.isOutputSoapEncoded(bindingOperation)) {
					...
					XmlObject[] paths = xml
							.selectPath("declare namespace env='" + wsdlContext.getSoapVersion().getEnvelopeNamespace()
					...
					// if( isResponse )
					// validateOutputAttachments( messageExchange, xml, errors,
					// bindingOperation, parts );
					// else
					// validateInputAttachments( messageExchange, errors,
					// bindingOperation, parts );
				} else {
	...
	private void validateDocLiteral(BindingOperation bindingOperation, Part[] parts, XmlObject msgXml,
		...
		if (elementName != null) {
			...
			if (paths.length == 1) {
				...
				errors.add(XmlError
						.forMessage("Missing message part with name:type [" + part.getName() + ":" + typeName + "]"));
		}
	}
	...
	private void validateMessageBody(List<XmlError> errors, SchemaType type, XmlObject msg) throws XmlException {
		...
		// reference wrong/non-existing namespace.. solution would probably be
		// to
		...
		if (bodyParts.length != 1) {
			...
			for (int i = 0; i < parts.length; i++) {
				...
				if (children.length != 1) {
					...
					QName elementName = part.getElementName();
					if (elementName != null) {
						bodyParts = msgXml.selectPath(
								"declare namespace env='" + wsdlContext.getSoapVersion().getEnvelopeNamespace() + "';"
	...
	private XmlObject[] getRpcBodyPart(BindingOperation bindingOperation, XmlObject msgXml, boolean isResponse)
			throws Exception {
		// rpc requests should use the operation name as root element and
		// soapbind
		// namespaceuri attribute as ns
		String ns = WsdlUtils
				.getSoapBodyNamespace(isResponse ? bindingOperation.getBindingOutput().getExtensibilityElements()
	...
	private void validateSoapFault(BindingOperation bindingOperation, XmlObject msgXml, List<XmlError> errors)
		...
		while (i.hasNext()) {
			...
			if (elementName != null) {
				XmlObject[] paths = msgXml
						.selectPath("declare namespace env='" + wsdlContext.getSoapVersion().getEnvelopeNamespace()
								+ "'; declare namespace flt='" + wsdlContext.getSoapVersion().getFaultDetailNamespace()
								+ "';" + "declare namespace ns='" + elementName.getNamespaceURI() + "';"
		...
		// if we get here, no matching fault was found.. this is not an error
		// but
		// should be warned..
		XmlObject[] paths = msgXml.selectPath("declare namespace env='"
				+ wsdlContext.getSoapVersion().getEnvelopeNamespace() + "'; declare namespace flt='"
	...
	// private void validateInputAttachments( WsdlMessageExchange
	// messageExchange, List<XmlError> errors,
	// BindingOperation bindingOperation, Part[] inputParts )
	// {
	// for( Part part : inputParts )
	// {
	// MIMEContent[] contents = WsdlUtils.getInputMultipartContent( part,
	// bindingOperation );
	// if( contents.length == 0 )
	// continue;
	//
	// Attachment[] attachments = messageExchange.getRequestAttachmentsForPart(
	// part.getName() );
	// if( attachments.length == 0 )
	// {
	// errors.add( XmlError.forMessage( "Missing attachment for part [" +
	// part.getName() + "]" ) );
	// }
	...
	// type.toUpperCase().startsWith( "MULTIPART" ) )
	// {
	// types = null;
	// break;
	// }
	...
	// if( types != null )
	// {
	// String msg = "Missing attachment for part [" + part.getName() + "] with
	// content-type [" + types + "],"
	...
	// if( types == null )
	// {
	// String msg = "Too many attachments for part [" + part.getName() + "] with
	// content-type [" + types + "]";
	// if(ALLOW_INCORRECT_CONTENT_TYPE)
	// log.warn( msg );
	// else
	...
	// }
	//
	// private void validateOutputAttachments( WsdlMessageExchange
	// messageExchange, XmlObject xml, List<XmlError> errors,
	// BindingOperation bindingOperation, Part[] outputParts ) throws Exception
	// {
	// for( Part part : outputParts )
	// {
	// MIMEContent[] contents = WsdlUtils.getOutputMultipartContent( part,
	// bindingOperation );
	// if( contents.length == 0 )
	// continue;
	//
	// Attachment[] attachments = messageExchange.getResponseAttachmentsForPart(
	// part.getName() );
	//
	// // check for rpc
	// if( attachments.length == 0 && WsdlUtils.isRpc(
	// wsdlContext.getDefinition(), bindingOperation ) )
	// {
	// XmlObject[] rpcBodyPart = getRpcBodyPart( bindingOperation, xml, true );
	// if( rpcBodyPart.length == 1 )
	// {
	// XmlObject[] children = rpcBodyPart[0].selectChildren( new QName(
	// part.getName() ) );
	// if( children.length == 1 )
	// {
	// String href = ( ( Element )children[0].getDomNode() ).getAttribute(
	// "href" );
	// if( href != null )
	// {
	...
	// }
	//
	// private void validateAttachmentsReadability( List<XmlError> errors,
