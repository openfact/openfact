...
import org.openfact.soa.common.annotation.ThreadSafe;
import org.openfact.soa.common.utils.Wsdl11Writer;
...
public class SoapMessageBuilder {
	...
	/**
	 * Constructs a new SoapBuilder instance importing the wsdl from the
	 * specified wsdlUrl. If the import is successful it saves the wsdl/xsd
	 * files to the target folder giving the top-level wsdl the specified
	 * baseName and setting the documentBaseUri to the newly saved wsdl uri. If
	 * the import is not successful an exception will be thrown and files will
	 * not be saved. Method expects that the targetFolder already exists.
	 *
	 * @param wsdlUrl
	 *            url of the wsdl to import
	 * @param targetFolder
	 *            folder in which all the files are be stored - folder has to
	 *            exist, no subfolders are created,
	 * @param fileBaseName
	 *            name of the top level file, without extension -> wsdl will be
	 *            added by default
	 * @return instance of the soap-builder which documentBaseUri is set to the
	 *         url of the locally saved wsdl
	 * @throws WSDLException
	 *             thrown in case of import errors
	 */
	public static SoapMessageBuilder createAndSave(URL wsdlUrl, File targetFolder, String fileBaseName)
			throws WSDLException {
	...
	private static void saveDefinition(String fileBaseName, Definition definition, File targetFolder) {
		if (targetFolder.exists() == false || targetFolder.isDirectory() == false) {
			throw new IllegalArgumentException(
					"Target folder does not exist or is not a folder [" + targetFolder.getPath() + "]");
		}
	...
	/**
	 * Saves wsdl recursively fetching all referenced wsdls and schemas fixing
	 * their location tags
	 *
	 * @param fileBaseName
	 *            name of the top level file, without extension -> wsdl will be
	 *            added by default
	 * @param targetFolder
	 *            folder in which all the files are stored - folder has to
	 *            exist, no subfolders are created,
	 */
		...
		if (elementName != null) {
			...
			if (definitionWrapper.hasSchemaTypes()) {
				...
				if (elm != null) {
					...
					log.error(
							"Could not find element [" + elementName + "] specified in part [" + part.getName() + "]");
			}
	...
	private void buildRpcRequest(BindingOperation bindingOperation, SoapVersion soapVersion, XmlCursor cursor,
		...
		if (ns == null) {
			...
			log.warn(
					"missing namespace on soapbind:body for RPC request, using targetNamespace instead (BP violation)");
		}
			...
			if (WsdlUtils.isAttachmentInputPart(part, bindingOperation)) {
				...
				if (definitionWrapper.hasSchemaTypes()) {
					...
					if (typeName != null) {
						...
						SchemaGlobalElement element = definitionWrapper.getSchemaTypeLoader()
	...
	private void buildRpcResponse(BindingOperation bindingOperation, SoapVersion soapVersion, XmlCursor cursor,
		...
		// rpc requests use the operation name as root element
		BindingOutput bindingOutput = bindingOperation.getBindingOutput();
		String ns = bindingOutput == null ? null
				: WsdlUtils.getSoapBodyNamespace(bindingOutput.getExtensibilityElements());

		if (ns == null) {
			ns = WsdlUtils.getTargetNamespace(definition);
			log.warn(
					"missing namespace on soapbind:body for RPC response, using targetNamespace instead (BP violation)");
		}
