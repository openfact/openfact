...
public class SoapContext {
	...
	/*
	 * A list of XML-Schema types and global elements in the form of
	 * name@namespace which will be excluded when generating sample requests and
	 * responses and input forms. By default the XML-Schema root element is
	 * added since it is quite common in .NET services and generates a sample
	 * xml fragment of about 300 kb!.
	 */
	private final Set<QName> excludedTypes;
	private final SoapMultiValuesProvider multiValuesProvider;

	/**
	 * Constructor mainly for SpringFramework purposes, in any other case use
	 * the fluent builder interface; #see builder() method
	 *
	 ...
	 * @param excludedTypes
	 */
	public SoapContext(boolean exampleContent, boolean typeComments, boolean valueComments, boolean buildOptional,
			boolean alwaysBuildHeaders, Set<QName> excludedTypes, SoapMultiValuesProvider multiValuesProvider) {
	...
	public static class ContextBuilder {
		...
		/**
		 * A list of XML-Schema types and global elements in the form of
		 * name@namespace which will be excluded when generating sample requests
		 * and responses and input forms. By default the XML-Schema root element
		 * is added since it is quite common in .NET services and generates a
		 * sample xml fragment of about 300 kb!.
		 *
		 * @param excludedTypes
		 * @return builder
		 */
		...
		public SoapContext build() {
			return new SoapContext(exampleContent, typeComments, valueComments, buildOptional, alwaysBuildHeaders,
					excludedTypes, multiValuesProvider);
		}
	}
